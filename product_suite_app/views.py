...
from product_suite_app.models import Product, ProductCategory
...
class AbstractFormView(ModelForm):
    ...
    def urls(cls):
        url = cls.Meta.url
        return [
            path(f'{url}', login_required(cls.list) if cls.Meta.auth else cls.list),
            path(f'{url}new/', login_required(cls.list) if cls.Meta.auth else cls.list),
            path(f'{url}<int:id>/', login_required(cls.list) if cls.Meta.auth else cls.list),
            path(f'{url}<int:id>/edit/', login_required(cls.list) if cls.Meta.auth else cls.list),
            path(f'{url}<int:id>/delete/', login_required(cls.list) if cls.Meta.auth else cls.list),
        ]
...
ProductCategoryForm = AbstractFormView.create_view(model_class=ProductCategory)
ProductForm = AbstractFormView.create_view(model_class=Product, model_fields_display=['name', 'file'])
